...
Grille= [0, 0, 0, 0,
         ...
         0, 0, 0, 0]
Entrée=["g","h","d","b"]
Couleurs=["white","green","blue","purple","pink","red","orange","yellow","grey","black"]
m="0"
p=0
    ...
    for y in range(0,4):
        for x in range(0,4):
            ...
            while a>=2:
                j=j+1
                a=a/2
            rectangle = canvas.create_rectangle((x+1/2)*Width//4-l+cx,(y+1/2)*Height//4-L+cy, (x+1/2)*Width//4+l+cx, (y+1/2)*Height//4+L+cy, fill=Couleurs[j])
            txt = canvas.create_text((x+1/2)*Width//4+cx,(y+1/2)*Height//4+cy, text=board[i], font="Arial 40 italic", fill=Couleurs[j+1])
            canvas.pack()
